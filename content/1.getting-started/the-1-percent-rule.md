...
**If there is even a 1% chance that a Gangsta Agents skill applies to what you are doing, you MUST invoke it.**
...
When you (the Don) express an intent, the agent evaluates whether any Gangsta Agents skill could apply. Even a slight possibility triggers invocation:
...
| "The Don wants it fast" | Speed without structure produces garbage. The Heist IS the fast path. |
...
**Example:** If you say "Build X," the agent first invokes `gangsta:reconnaissance` (process), then during the Execution phase will use `gangsta:drill-tdd` (implementation) for code writing.
::
...
The 1% Rule is what separates Gangsta Agents-assisted development from undisciplined chat-driven coding. Respect it, and your projects will be spec'd before they're built.
